...
# 1. Add Sidebar Link
sidebar_target = '<div class="sb-link" data-module="site" onclick="switchTab(\'news\')"><i class="fas fa-newspaper"></i> <span>Xəbər & Media</span></div>'
sidebar_replacement = sidebar_target + '\n      <div class="sb-link" data-module="site" onclick="switchTab(\'popup\')"><i class="fas fa-window-restore"></i> <span>POP UP</span></div>'
if 'switchTab(\'popup\')' not in content:
    content = content.replace(sidebar_target, sidebar_replacement)

# 2. Update titles JS
titles_target = "'news':'Xəbərlər',"
titles_replacement = "'news':'Xəbərlər', 'popup':'POP UP Yönətimi',"
if "'popup':'POP UP Yönətimi'" not in content:
    content = content.replace(titles_target, titles_replacement)

# 3. Add view-popup HTML
view_news_regex = r'(<div id="view-news".*?</div>\s*</div>\s*</div>\s*</div>)'
# Actually, it's safer to append it before `<!-- MCP MODULE -->` or `<!-- SETTINGS MODULE -->`
html_injection = """
    <!-- POPUP MODULE -->
...
if '<div id="view-popup"' not in content:
    content = content.replace('<!-- SETTINGS MODULE -->', html_injection + '\n    <!-- SETTINGS MODULE -->')
...
js_injection = """
    ...
    const popupFileUploader = document.getElementById('popup-img-file');
    if (popupFileUploader) {
        popupFileUploader.addEventListener('change', function() {
            const file = this.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = function(e) { document.getElementById('popup-img').value = e.target.result; };
    ...
    window.handlePopupSubmit = async (e) => {
        ...
        // If active, it's good practice to maybe deactivate others, but we'll let user manage it.
...
if 'window.loadPopups =' not in content:
    # Insert JS logic before the closing script tag of the main js block
    # Let's find loadNews() call and append loadPopups() to it
    ...
    content = content.replace('// --- NEWS LOGIC ---', js_injection + '\n\n    // --- NEWS LOGIC ---')


with open(file, 'w', encoding='utf-8') as f:
    f.write(content)

print("POP UP admin panel logic injected successfully.")
